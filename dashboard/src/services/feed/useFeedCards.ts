...
/**
 * Restrict cards to a set of ISO2 country codes. Single source of truth for the
 * country filter. No filter (null/empty) → all cards, unchanged. Cards with no
 * resolved country are dropped only when a filter is active.
 */
export function filterFeedCardsByCountry(
  cards: FeedCard[],
  countryFilter?: readonly string[] | null,
): FeedCard[] {
  if (!countryFilter || countryFilter.length === 0) return cards;
  const set = new Set(countryFilter);
  return cards.filter((c) => c.country != null && set.has(c.country));
}
